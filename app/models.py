...
class Role(db.Model):
    ...
    def insert_roles():
        ...
        for name_role, value in roles.items():
...
class User(UserMixin, db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.role:
            ...
            if not self.role:
                # Задаем роль 'User' для пользователя
...
class Post(db.Model):
    ...
    def on_changed_body(target, value, oldvalue, initiator):
        allowed_tags = [
            ...
            'h1', 'h2', 'h3', 'p', 'img']
        initial_body_html = markdown(value, output_format='html')
        target.body_html = linkify(clean(initial_body_html, tags=allowed_tags, strip=True))
...
loging_manager.anonymous_user = AnonymousUser
